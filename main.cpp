...
int main() {
    ...
    // Blocks chosen to match the default heuristic used by the wrapper for this dataset (1,1,1), init_mem=8.
    container_poly con(0.0, L, 0.0, L, 0.0, L,
                       1, 1, 1,
                       true, true, true,
                       8);
    ...
    // Print detailed info for pid 0 and pid 9 (pid 9 is the problematic cell on macOS arm64).
    ...
    for (int target : {0, 9}) {
        ...
        if (!found) {
            std::cerr << "Could not find pid=" << target << " in loop\n";
            return 3;
        }
    }

    // Simple reciprocity check between pid 0 and pid 9.
